...
describe('ServerDetails', () => {
  ...
  it('renders server groups in semantic order with null values and links', () => {
    ...
    expect(publicIpDl.compareDocumentPosition(copyControl) & Node.DOCUMENT_POSITION_FOLLOWING).toBeTruthy();
    expect(
    ...
    expect(Array.from(headerSection.querySelectorAll('dt')).map((term) => term.textContent)).toEqual(
      Object.keys(inspection.headers),
    );
  });
});
