...
describe('HomeView', () => {
  ...
  it('renders the complete unstyled semantic page in the required heading order', () => {
    ...
    const sections = screen.getAllByRole('heading').slice(1).map((heading) => heading.closest('section'));
  ...
  it('integrates browser details and an IP copy control while retaining server links', async () => {
    ...
    const requestHeaders = screen.getByRole('heading', { name: 'Request headers', level: 2 }).closest('section');
    if (!requestHeaders) {
